...
class DefaultNotificationSerializationServiceTest {
    ...
    void testSerialization() {
        ...
        assertTrue(json.contains("\"tag\":[\"etag:1234567\"]"), "tag not in serialization!");
    }

    @Test
    void testError() {
        when(mockNotification.getIdentifier()).thenAnswer(inv -> {
            sneakyJsonException();
            return rdf.createIRI("info:notification/12456");
        });

        assertThrows(TrellisRuntimeException.class, () -> svc.serialize(mockNotification));
    }
